...
function Escrow(tx) {
  ...
  escrow.amount = tx.Amount || node.FinalFields.Amount
  if (typeof escrow.amount === 'object') {
    escrow.amount = JSON.stringify(escrow.amount)
  } else {
    escrow.amount = new BigNumber(escrow.amount).dividedBy(XRP_ADJUST).toString()
  }
