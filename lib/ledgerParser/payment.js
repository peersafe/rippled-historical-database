...
var parseBalanceChanges   = require('chainsql-lib-transactionparser').parseBalanceChanges;
var parseOrderBookChanges = require('chainsql-lib-transactionparser').parseOrderBookChanges;
...
var Payments = function (tx) {
  ...
  balanceChanges[tx.Account].forEach(function(change) {
    if (change.currency === 'ZXC') {
      var fee      = new BigNumber (tx.Fee).dividedBy(ZXC_ADJUST).negated();
  ...
  if (typeof tx.Amount === 'object') {
    ...
    payment.currency = 'ZXC';
    payment.amount   = new BigNumber(tx.Amount).dividedBy(ZXC_ADJUST).toString();
  }
  ...
  if (typeof amount === 'object') {
    ...
    payment.delivered_amount = new BigNumber(amount).dividedBy(ZXC_ADJUST).toString();
  }
  ...
  if (typeof tx.SendMax === 'object') {
    ...
    payment.max_amount      = new BigNumber(tx.SendMax).dividedBy(ZXC_ADJUST).toString();
    payment.source_currency = 'ZXC';
  }
  ...
  payment.fee = new BigNumber (tx.Fee).dividedBy(ZXC_ADJUST).toString();
