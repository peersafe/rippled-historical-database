...
var OffersExercised = function (tx) {
  ...
  if (tx.TransactionType !== 'Payment' && tx.TransactionType !== 'OfferCreate' && tx.TransactionType !== 'OfferCancel') {
    return list;
  }
  ...
  function parseOfferExercised (node, tx) {
    ...
    // TakerPays DAC
      ...
      base = {
        currency: 'DAC',
        amount: change.dividedBy(DAC_ADJUST).toString()
      }
    }
