...
var BalanceChanges = function (tx) {
  ...
  function parseAccountRoot (node) {
    ...
    if (tx.Account === account) {
      ...
      list.push({
        account       : account,
        currency      : 'DAC',
        change        : fee.dividedBy(DAC_ADJUST).toString(),
        final_balance : balance.minus(amount).dividedBy(DAC_ADJUST).toString(),
    ...
    if (!amount.isZero()) {
      data = {
        account       : account,
        currency      : 'DAC',
        change        : amount.dividedBy(DAC_ADJUST).toString(),
        final_balance : balance.dividedBy(DAC_ADJUST).toString(),
        time          : tx.executed_time,
        ledger_index  : tx.ledger_index,
        tx_index      : tx.tx_index,
  ...
  function findType (data) {
    ...
    //offer creates are all exchanges
      ...
      // payment currency = DAC and destination account
      } else if (data.account === tx.Destination &&
                 !tx.Amount.currency &&
                 data.currency === 'DAC') {
        return 'payment_destination';
      ...
      // source currency = DAC and source account
      } else if (data.account === tx.Account &&
                tx.SendMax &&
                data.currency === 'DAC') {
        return 'payment_source';
