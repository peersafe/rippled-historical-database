...
var BalanceChanges = function (tx) {
  ...
  function parseAccountRoot (node) {
    ...
    if (tx.Account === account) {
      ...
      list.push({
        account       : account,
        currency      : 'ZXC',
        change        : fee.dividedBy(ZXC_ADJUST).toString(),
        final_balance : balance.minus(amount).dividedBy(ZXC_ADJUST).toString(),
    ...
    if (!amount.isZero()) {
      data = {
        account       : account,
        currency      : 'ZXC',
        change        : amount.dividedBy(ZXC_ADJUST).toString(),
        final_balance : balance.dividedBy(ZXC_ADJUST).toString(),
        time          : tx.executed_time,
        ledger_index  : tx.ledger_index,
        tx_index      : tx.tx_index,
  ...
  function findType (data) {
    ...
    //offer creates are all exchanges
      ...
      // payment currency = ZXC and destination account
      } else if (data.account === tx.Destination &&
                 !tx.Amount.currency &&
                 data.currency === 'ZXC') {
        return 'payment_destination';
      ...
      // source currency = ZXC and source account
      } else if (data.account === tx.Account &&
                tx.SendMax &&
                data.currency === 'ZXC') {
        return 'payment_source';
