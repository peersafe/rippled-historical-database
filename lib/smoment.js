...
module.exports = function(time) {
  ...
  if(m) {
    ...
    m.addByMinute = function (v) {
      var time = moment(this.moment).add(v, 'minute');
      return time.format();
    }
    m.addByMonth = function (v) {
      var time = moment(this.moment).add(v, 'month');
      return time.format();
    }
    m.addByYear = function (v) {
      var time = moment(this.moment).add(v, 'year');
      return time.format();
    }

    m.subtractByMinute = function (v) {
      var time = moment(this.moment).subtract(v, 'minute');
      return time.format();
    }
    m.subtractByMonth = function (v) {
      var time = moment(this.moment).subtract(v, 'month');
      return time.format();
    }
    m.subtractByYear = function (v) {
      var time = moment(this.moment).subtract(v, 'year');
      return time.format();
    }
