...
HbaseClient.prototype.getScan = function(options, callback) {
    ...
    self._getConnection(function (err, connection) {
         ...
         * getResults
         */
        function getResults(id, onlyCount, l, cb) {
            var batchSize = 5000
            var page = 1
            var results = []
            var limit = onlyCount ? undefined : l;
            if (onlyCount) results[0] = 0;
            self.log.info(limit);
            self.log.info(onlyCount);
            ...
            function recursiveGetResults() {
                ...
                connection.client.scannerGetList(id, count, function (err2, rows) {
                    if (rows && rows.length) {
                        self.log.info("rows.length:", rows.length);
                        if (onlyCount) results[0] += rows.length;
                        else {
                            // add to the list
        ...
        self.log.info("scanOpts:", JSON.stringify(scanOpts));
        scan = new HBaseTypes.TScan(scanOpts)
        ...
        connection.client.scannerOpenWithScan(table, scan, null,
            function (err2, id) {
                ...
                getResults(id, options.onlyCount, options.limit, function (err3, rows) {
