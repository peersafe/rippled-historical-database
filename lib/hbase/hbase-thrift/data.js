...
HbaseClient.peak_lgr_index = 0;
HbaseClient.peak_tx_num    = 0;

HbaseClient.initPeakInfo = function() {
    let options = {
        peak : true
    }
    this.getLedger(options, function(err, ret) {
        if(err) {
            console.error(err);
        } else {
            if(ret !== null) {
                HbaseClient.peak_lgr_index = ret.peak_ledger_index;
                HbaseClient.peak_tx_num = ret.peak_tx_num;
            }
        }
    });
}
...
HbaseClient.getExchangeRate = function(options) {
  ...
  if (!options.base) {
    options.base = {currency: 'ZXC'}
  } if (!options.counter) {
    options.counter = {currency: 'ZXC'}
  }
...
HbaseClient.getExchanges = function(options, callback) {
  ...
  */
  ...
  function reduce(data) {
    ...
    // filter out small ZXC amounts
    rows = rows.filter(function(row) {
      if (options.base.currency === 'ZXC' &&
          row.base_amount < 0.0005) {
        return false
      } else if (options.counter.currency === 'ZXC' &&
...
HbaseClient.getAccountExchanges = function(options, callback) {
  ...
  if (options.counter.currency < options.base.currency) {
    swap = options.base
    options.base = options.counter
    options.counter = swap
    options.invert = true
  } 

  function invertPair(row) {
    row.rate = 1 / row.rate

    // swap base and counter vol
    swap = row.base_amount
    row.base_amount = row.counter_amount
    row.counter_amount = swap
    ...
    swap = row.base_currency
    row.base_currency = row.counter_currency
    row.counter_currency = swap

    swap = row.base_issuer
    row.base_issuer = row.counter_issuer
    row.counter_issuer = swap
  }
  ...
  this.getScanWithMarker(this, {
    ...
    startRow: options.account + '|' + utils.reverseFormatTime(options.start.hbaseFormatStartRow()),
    stopRow: options.account + '|' + utils.reverseFormatTime(options.end.hbaseFormatStopRow()),
    descending: options.descending,
    limit: options.limit,
    filterString: filterString,
    marker: options.marker
...
HbaseClient.getLedger = function(options, callback) {
  ...
  // get by index, or get latest
    ...
    } else if (options.peak) {
        self.log.info("resp:", JSON.stringify(resp[0]))
        let history_peak = {};
        history_peak.peak_ledger_index = resp[0].peak_lgr_index;
        history_peak.peak_tx_num = resp[0].peak_tx_num;
        callback(null, history_peak);
        return;
      } else {
        options.ledger_hash = resp[0].ledger_hash
      }
...
HbaseClient.getTransactions = function(options, callback) {
  ...
  function getTransactionsByTime(opts, cb) {
    ...
    const oneKeyFilterStr = 'FirstKeyOnlyFilter() AND KeyOnlyFilter()'
    self.log.info("opts.limit:", opts.limit);
    self.log.info("opts.marker:", opts.marker);
    self.getScanWithMarker(self, {
      table: 'lu_transactions_by_time',
      startRow: opts.start.hbaseFormatStartRow(),
      stopRow: opts.end.hbaseFormatStopRow(),
      onlyCount: opts.onlyCount,
      marker: opts.onlyCount ? false : opts.marker,
      descending: opts.descending,
      limit: opts.limit,
      filterString: opts.onlyCount ? oneKeyFilterStr : self.buildSingleColumnValueFilters(filters),
      columns: opts.onlyCount ? false : ['d:tx_hash', 'f:type', 'f:result']
    }, function(err, resp) {

        if (resp && !opts.onlyCount) {
            resp.rows.forEach(function(row, i) {
                resp.rows[i] = row.tx_hash
            })
        }
        if(opts.onlyCount) {
            resp.start = opts.start.moment;
            resp.end = opts.end.moment;
        }
  ...
  if (options.hashes) {
    ...
    getTransactionsByTime(options, function(err, resp) {
      ...
      if (err) {
        ...
        options.onlyCount ? callback(null, resp) : getTransactionsFromHashes(options, callback)
...
HbaseClient.prepareLedgerTables = function(ledger) {
  ...
  var ledgerTimeKey = utils.formatTime(ledger.close_time) +
    ...
    console.log("ledger.transactions.length:", ledger.transactions.length);
    console.log("ledger.ledger_index:", ledger.ledger_index);
    console.log("HbaseClient.peak_tx_num:", HbaseClient.peak_tx_num);
    console.log("HbaseClient.peak_lgr_index:", HbaseClient.peak_lgr_index);

    const historyPeakTxNumTmp = HbaseClient.peak_tx_num;
    HbaseClient.peak_tx_num = ledger.transactions.length > historyPeakTxNumTmp ? 
                            ledger.transactions.length : HbaseClient.peak_tx_num;
    HbaseClient.peak_lgr_index = ledger.transactions.length > historyPeakTxNumTmp ? 
                            ledger.ledger_index : HbaseClient.peak_lgr_index;
  ...
  tables.lu_ledgers_by_index[ledgerIndexKey] = {
    ...
    'f:peak_lgr_index': HbaseClient.peak_lgr_index,
    'f:peak_tx_num': HbaseClient.peak_tx_num
  }
...
HbaseClient.prepareParsedData = function(data) {
  ...
  data.exchanges.forEach(function(ex) {
    ...
    suffix = utils.reverseTimestamp(ex.time) +
      '|' + utils.padNumber(ex.ledger_index, LI_PAD) +
      '|' + utils.padNumber(ex.tx_index, I_PAD) +
      '|' + utils.padNumber(ex.node_index, I_PAD) // guarantee uniqueness
